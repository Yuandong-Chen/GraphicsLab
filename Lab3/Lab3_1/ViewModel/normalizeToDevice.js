...
let tryFillPoly = (height, width, poly, dataset, points) => {
  ...
  for (let i = 0; i < poly.length; i++) {
    ...
    let I_fst = 0.01 + vecDotMul(points[poly[i % poly.length]][0], L) +  Math.pow(vecDotMul(points[poly[i % poly.length]][0], H), 2)
    let I_snd = 0.01 + vecDotMul(points[poly[(i + 1) % poly.length]][0], L) + Math.pow(vecDotMul(points[poly[(i + 1) % poly.length]][0], H), 2)
  ...
  while(AET.length > 0 && beginY + YMIN < height) {
    ...
    for (let i = 0; i + 1 < AET.length; i+=2) {
      ...
      let ly1 = larr1[0] + (larr1[1] - larr1[0]) * (beginY + YMIN - larr1[2])/(Ymax1 - larr1[2])
      let ly2 = larr2[0] + (larr2[1] - larr2[0]) * (beginY + YMIN - larr2[2])/(Ymax2 - larr2[2])
      ...
      for (let j = Math.floor(Xmin1); j <= Math.floor(Xmin2); j++) {
        ...
        if(beginY + YMIN >= 0 && beginY + YMIN < height && j >= 0 && j < width) {
          ...
          if(curZ < dataset[j][beginY + YMIN][0]) {
            let I = (j - Math.floor(Xmin1))/(Math.floor(Xmin2) - Math.floor(Xmin1)) * (ly2 - ly1) + ly1
